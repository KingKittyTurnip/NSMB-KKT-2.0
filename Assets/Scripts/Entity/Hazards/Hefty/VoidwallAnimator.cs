...
namespace NSMB.Entities.Player {
    public class VoidwallAnimator : QuantumEntityViewComponent {
        ...
        [SerializeField] private Renderer voidwallRenderer;
        private MaterialPropertyBlock materialBlock;

        private static readonly int ParamHide = Shader.PropertyToID("Hide");
        ...
        public override unsafe void OnUpdateView() {
            ...
            if (WallIsSeeThrough) {
                ...
                //Set Transparancy
                materialBlock.SetFloat("Hide", 0.2f);
                voidwallRenderer.SetPropertyBlock(materialBlock);
            } else {
                ...
                bool flip = !(OurPos.x < CameraPos.x || ExpandTimer == 0);
                sRenderer.transform.localPosition = new Vector3((OurPos.x < CameraPos.x ? 0.22f : -0.22f) - (flip ? 0 : Xsize*0.5f), 0, -9f);

                //Set Transparancy
                materialBlock.SetFloat("Hide", (voidwall->DamageCooldown.AsFloat == 0) ? 0 : 0.8f);
                voidwallRenderer.SetPropertyBlock(materialBlock);

                //render over further away void walls in the case there's multiple to prevent z fighting
                sRenderer.sortingOrder = 1008 + (24 - Mathf.RoundToInt(Mathf.Abs(CameraPos.x - OurPos.x)*4));
            }

            //Set Sprite
            bool CurrentlyDamaged = !(voidwall->DamageCooldown.AsFloat > 0 && (f.Number * f.DeltaTime.AsFloat) * 3 % 0.2f < 0.1f);
