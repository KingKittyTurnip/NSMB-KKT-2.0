...
namespace Quantum {
    public unsafe partial struct MarioPlayer {
        ...
        public readonly bool IsValid(Frame f) => !Disconnected && !(f.Global->Rules.ModifierLivesEnabled && Lives == 0);
        ...
        public readonly bool CanHoldItem(Frame f, EntityRef entity, EntityRef item) {
            ...
            return (input.Sprint.IsDown || forceHold || (f.Exists(HeldEntity) && !f.IsPlayerVerifiedOrLocal(PlayerRef)))
                ...
                && !IsInKnockback && KnockbackGetupFrames == 0 && !IsTurnaround && ((!IsPropellerFlying && !IsSpinnerFlying) || PropellerBux) && !(IsCrouching && !BillBux) && !IsDead
