      ...
      //TODO: Get Rid Of Bad Powerups (Ex: Mini & Jumpsuit)
        ...
        private void HandleJumping(Frame f, ref Filter filter, MarioPlayerPhysicsInfo physics) {
            ...
            if (canSpecialJump && mario->JumpState == JumpState.SingleJump) {
                ...
                newY += mario->CurrentPowerupState == PowerupState.JumpSuit ? physics.JumpTripleBonusVelocity * 6 : physics.JumpTripleBonusVelocity;
            } else {
                // Normal jump
        ...
        public void HandleGravity(Frame f, ref Filter filter, MarioPlayerPhysicsInfo physics) {
            ...
            } else if (mario->CanWallClimb && mario->IsWallsliding && physicsObject->Velocity.Y <= Constants._3_50) {
                gravity = 0;
            } else {
        ...
        private void HandleWallslideStopChecks(ref Filter filter, FPVector2 wallDirection) {
            ...
            // Cat Climb Checks
            if (mario->CanWallClimb) {
              if (mario->ClimbFrames == 90)
                  mario->WeakClimb = false;
              QuantumUtils.Decrement(ref mario->ClimbFrames);

              //Temp Variables
              physicsObject->Velocity.X = mario->WallslideRight ? FP._0_25 : -FP._0_25;
              bool ClimbDown = inputs.Down.IsDown;
              bool ClimbUp = inputs.Up.IsDown || (inputs.Left.IsDown && mario->WallslideLeft) || (inputs.Right.IsDown && mario->WallslideRight);

              //Stop Climbing
              if (physicsObject->IsTouchingGround || (inputs.Left.IsDown && mario->WallslideRight) || (inputs.Right.IsDown && mario->WallslideLeft) || (!physicsObject->IsTouchingLeftWall && !physicsObject->IsTouchingRightWall)) {
                  mario->WallslideRight = false;
                  mario->WallslideLeft = false;
                  mario->WallslideEndFrames = 0;
                  mario->ClimbFrames = 0;
                  mario->JumpState = JumpState.SingleJump;
                  mario->WeakClimb = true;
              } else if (physicsObject->Velocity.Y <= Constants._3_50) {
                  physicsObject->Velocity.Y = (mario->ClimbFrames < 15 ? 0 : 1) * ((ClimbUp ? 1 : 0) + (ClimbDown ? -1 : 0)) * Constants._3_50; // 3.5f (Point Of Refrence)
              }
              return;
            }
        ...
        public void HandleKnockback(Frame f, ref Filter filter) {
            ...
            if (mario->IsInKnockback) {
                ...
                if (mario->DoEntityBounce
                    ...
                    || (!swimming && mario->InKnockback == KbType.Weak && framesInKnockback > 30)) {
        ...
        private void HandlePowerups(Frame f, ref Filter filter, MarioPlayerPhysicsInfo physics, VersusStageData stage) {
            ...
            physicsObject->IsWaterSolid = mario->CurrentPowerupState == PowerupState.MiniMushroom && !mario->IsGroundpounding && mario->StationaryFrames < 15 && (!mario->IsInKnockback || mario->InKnockback == KbType.Weak);
            ...
            case PowerupState.HammerSuit: {
                ...
                if (activeProjectiles >= (mario->CurrentPowerupState == PowerupState.HammerSuit ? 2 : physics.MaxProjecitles)) {
                    return;
                }
            ...
            case PowerupState.PropellerMushroom: {
                ...
                physicsObject->IsTouchingGround = false;
                physicsObject->HoverFrames = 0;
                PhysicsObjectSystem.MoveVertically((FrameThreadSafe) f, FPVector2.Up * FP._0_05 * f.UpdateRate, filter.Entity, stage, default, out _);
...
QuantumUtils.Decrement(ref mario->PipeFrames);
        ...
        private void HandleSpinners(Frame f, ref Filter filter, VersusStageData stage) {
            ...
            if (FPMath.Abs(moveVelocity) > 0) {
                PhysicsObjectSystem.MoveHorizontally((FrameThreadSafe) f, new FPVector2(moveVelocity, 0), filter.Entity, stage, contacts, out _);
            }
        }
            ...
            if (!mario->IsInKnockback
                ...
                case ProjectileEffectType.Fire:
                    if (dropStars && mario->CurrentPowerupState == PowerupState.MiniMushroom) {
                        ...
                        mario->DoKnockback(f, marioEntity, !projectile->FacingRight, dropStars ? 1 : 0, 
                            dropStars ? KbType.Fireball : KbType.Weak, projectileEntity);
                    }
                    break;
                case ProjectileEffectType.Freeze:
                    if (dropStars && mario->CurrentPowerupState == PowerupState.MiniMushroom) {
                        ...
                        mario->DoKnockback(f, marioEntity, !projectile->FacingRight, dropStars ? 1 : 0, KbType.Weak, projectileEntity);
                    }
                    break;
                }
            }
        ...
        public static void OnMarioMarioInteraction(Frame f, EntityRef marioAEntity, EntityRef marioBEntity) {
            ...
            if ((f.Number - marioA->KnockbackTick) < 16 || (f.Number - marioB->KnockbackTick) < 16) {
                return;
            }
            ...
            if (marioAStarman && marioBStarman) {
                marioA->DoKnockback(f, marioAEntity, fromRight, dropStars ? 1 : 0, KbType.Weak, marioBEntity);
                marioB->DoKnockback(f, marioBEntity, !fromRight, dropStars ? 1 : 0, KbType.Weak, marioAEntity);
                return;
            ...
            if (marioAMega && marioBMega) {
                ...
                if (marioAAbove) {
                    ...
                    marioA->DoKnockback(f, marioAEntity, fromRight, 0, KbType.Weak, marioBEntity);
                    marioB->DoKnockback(f, marioBEntity, !fromRight, 0, KbType.Weak, marioAEntity);
                }
                return;
            } else if (marioAMega) {
                if (dropStars) {
            ...
            if (marioAShell && marioBShell) {
                ...
                if (!marioBAbove) {
                    ...
                    if (dropStars) {
                        marioB->Powerdown(f, marioBEntity, false);
                        marioB->DoKnockback(f, marioBEntity, !fromRight, 0, KbType.Stomp, marioAEntity);
                    } else {
                        marioB->DoKnockback(f, marioBEntity, !fromRight, 0, KbType.Stomp, marioAEntity);
                    }
                ...
                if (!marioAAbove) {
                    ...
                    if (dropStars) {
                        marioA->Powerdown(f, marioAEntity, false);
                        marioA->DoKnockback(f, marioAEntity, !fromRight, 0, KbType.Stomp, marioBEntity);
                    } else {
                        marioA->DoKnockback(f, marioAEntity, !fromRight, 0, KbType.Stomp, marioBEntity);
                    }
            ...
            if (marioAMini ^ marioBMini && collidable) {
                // Minis
                if (marioAMini) {
                    marioA->DoKnockback(f, marioAEntity, fromRight, dropStars ? 1 : 0, KbType.Stomp, marioBEntity);
                }
                if (marioBMini) {
                    marioB->DoKnockback(f, marioBEntity, !fromRight, dropStars ? 1 : 0, KbType.Stomp, marioAEntity);
                }
            ...
            } else if (collidable && !marioA->IsInKnockback && !marioB->IsInKnockback) {
                // Collide
                ...
                if (overlap > 0) {
                    // Move 
                    PhysicsObjectSystem.MoveHorizontally((FrameThreadSafe) f, new FPVector2(overlap * directionToOtherPlayer * f.UpdateRate, 0), marioAEntity, stage, default, out _);
                    PhysicsObjectSystem.MoveHorizontally((FrameThreadSafe) f, new FPVector2(overlap * -directionToOtherPlayer * f.UpdateRate, 0), marioBEntity, stage, default, out _);
        ...
        private static void MarioMarioAttackStarman(Frame f, EntityRef attacker, EntityRef defender, bool fromRight, bool dropStars) {
            ...
            if (defenderMario->CurrentPowerupState == PowerupState.MegaMushroom) {
                // Wait fuck-
                attackerMario->DoKnockback(f, attacker, fromRight, dropStars ? 1 : 0, KbType.Weak, defender);
            } else {
                if (dropStars) {
                    defenderMario->Powerdown(f, defender, false);
                } else {
                    defenderMario->DoKnockback(f, defender, !fromRight, 0, KbType.Weak, attacker);
                }
            }
        }
        ...
        private static void MarioMarioStomp(Frame f, EntityRef attacker, EntityRef defender, bool fromRight, bool dropStars) {
            ...
            //Attacker In Knockback, Don't Hit
            if (attackerMario->IsInKnockback || attackerMario->KnockbackGetupFrames > 0)
                return;
            ...
            //Ignore Mini interactions For mini Fusions
            if (attackerMario->CurrentPowerupState == PowerupState.MiniMushroom && defenderMario->CurrentPowerupState != PowerupState.MiniMushroom) {
                ...
                if (groundpounded) {
                    defenderMario->DoKnockback(f, defender, !fromRight, dropStars ? 3 : 0, KbType.Stomp, attacker);
                ...
                if (defenderMario->CurrentPowerupState == PowerupState.MiniMushroom && groundpounded) {
                    ...
                    if (!groundpounded && !dropStars) {
                        ...
                        if (attackerMario->IsPropellerFlying && attackerMario->IsDrilling) {
                            ...
                            attackerMario->DoEntityBounce = true;
                        } //Make Mini Fusions drop 1 less star with gps
                        defenderMario->DoKnockback(f, defender, !fromRight, dropStars ? (groundpounded ? 3 : 1) : 0, KbType.Stomp, attacker);
                    }
                }
            }
        }
        ...
        public void OnEntityBumped(Frame f, EntityRef entity, FPVector2 tileWorldPosition, EntityRef bumper) {
            if (f.Unsafe.TryGetPointer(entity, out MarioPlayer* mario)) {
                ...
                mario->DoKnockback(f, entity, !onRight, 1, KbType.Stomp, bumper);
            }
        }
        ...
        public void OnIceBlockBroken(Frame f, EntityRef brokenIceBlock, IceBlockBreakReason breakReason) {
            ...
            case IceBlockBreakReason.Other:
                ...
                mario->DoKnockback(f, entity, mario->FacingRight, 1, KbType.Weak, brokenIceBlock);
                break;

            case IceBlockBreakReason.Groundpounded:
                // Hard knockback, 2 stars
                mario->DoKnockback(f, entity, mario->FacingRight, 2, KbType.Stomp, brokenIceBlock);
                break;
