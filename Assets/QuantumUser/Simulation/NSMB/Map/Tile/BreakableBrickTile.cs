...
public unsafe class BreakableBrickTile : StageTile, IInteractableTile {
    ...
    public virtual bool Interact(Frame f, EntityRef entity, InteractionDirection direction, IntVector2 tilePosition, StageTileInstance tileInstance, out bool playBumpSound) {
        ...
        if (f.Unsafe.TryGetPointer(entity, out MarioPlayer* mario)) {
        ...
        } else if (f.Unsafe.TryGetPointer(entity, out Starball* starball)) {
            doBreak = f.Unsafe.GetPointer<PhysicsObject>(entity)->BreakMegaObjects ? BreakingRules.HasFlag(BreakableBy.MegaMario) : BreakingRules.HasFlag(BreakableBy.Shells);
            doBump = true;
            ...
            doBump = false;
        } else if (f.Unsafe.TryGetPointer(entity, out Thwomp* thwomp)) {
            doBreak = thwomp->Big ? BreakingRules.HasFlag(BreakableBy.MegaMario) : BreakingRules.HasFlag(BreakableBy.Shells);
            doBump = false;
            bumpOwner = entity;
            allowSelfDamage = false;
        }
